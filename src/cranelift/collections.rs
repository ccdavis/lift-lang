...
impl<'a, M: Module> CodeGenerator<'a, M> {
    pub(super) fn compile_list_literal(
        ...
        let type_tag = builder
            .ins()
            .iconst(types::I8, data_type_to_type_tag(&elem_type) as i64);
        ...
        for (i, elem) in data.iter().enumerate() {
            ...
            let elem_val = match elem_type {
                DataType::Flt => {
                    ...
                    builder
                        .ins()
                        .bitcast(types::I64, MemFlags::new(), elem_val_raw)
                }
                DataType::Bool => {
    ...
    pub(super) fn compile_map_literal(
        ...
        if !matches!(
            actual_key_type,
            DataType::Int | DataType::Bool | DataType::Str
        ) {
            return Err(format!(
                "Map keys must be scalar types (Int, Bool, or Str), got {:?}",
                actual_key_type
            ));
        }
        ...
        let capacity = builder.ins().iconst(types::I64, data.len() as i64);
        let key_type_tag = builder
            .ins()
            .iconst(types::I8, data_type_to_type_tag(&actual_key_type) as i64);
        let value_type_tag = builder
            .ins()
            .iconst(types::I8, data_type_to_type_tag(&actual_value_type) as i64);
        let func_ref = runtime_funcs
            .get("lift_map_new")
            .ok_or_else(|| "Runtime function lift_map_new not found".to_string())?;
        let inst = builder
            .ins()
            .call(*func_ref, &[capacity, key_type_tag, value_type_tag]);
        ...
        for (key_data, value_expr) in data {
            ...
            let value_val_raw = Self::compile_expr_static(
                builder,
                value_expr,
                symbols,
                runtime_funcs,
            ...
            let value_val = match actual_value_type {
                DataType::Flt => {
                    ...
                    builder
                        .ins()
                        .bitcast(types::I64, MemFlags::new(), value_val_raw)
                }
                DataType::Bool => {
                    // Bool is already I64 in our representation
                    value_val_raw
    ...
    pub(super) fn compile_index(
        ...
        let collection = Self::compile_expr_static(
            builder,
            expr,
            symbols,
            runtime_funcs,
        ...
        let index_val = Self::compile_expr_static(
            builder,
            index,
            symbols,
            runtime_funcs,
            ...
            DataType::List { element_type } => {
                ...
                let result = match *element_type {
                    DataType::Flt => {
                        ...
                        builder
                            .ins()
                            .bitcast(types::F64, MemFlags::new(), result_i64)
                    }
                    DataType::Bool
                    | DataType::Int
                    | DataType::Str
