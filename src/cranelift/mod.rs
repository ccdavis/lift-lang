...
pub(crate) fn data_type_to_type_tag(data_type: &crate::syntax::DataType) -> i8 {
...
impl<'a, M: Module> CodeGenerator<'a, M> {
    ...
    pub fn declare_runtime_functions(&mut self) -> Result<(), String> {
        ...
        self.runtime_funcs.insert("lift_struct_new".to_string(), func_id);
        ...
        let func_id = self.module
            .declare_function("lift_struct_set_field", cranelift_module::Linkage::Import, &sig)
            .map_err(|e| format!("Failed to declare lift_struct_set_field: {}", e))?;
        self.runtime_funcs.insert("lift_struct_set_field".to_string(), func_id);
        ...
        sig.returns.push(AbiParam::new(types::I64));
        let func_id = self.module
            .declare_function("lift_struct_get_field", cranelift_module::Linkage::Import, &sig)
            .map_err(|e| format!("Failed to declare lift_struct_get_field: {}", e))?;
        self.runtime_funcs.insert("lift_struct_get_field".to_string(), func_id);
        ...
        sig.returns.push(AbiParam::new(types::I8));
        let func_id = self.module
            .declare_function("lift_struct_get_field_type", cranelift_module::Linkage::Import, &sig)
            .map_err(|e| format!("Failed to declare lift_struct_get_field_type: {}", e))?;
        self.runtime_funcs.insert("lift_struct_get_field_type".to_string(), func_id);
        ...
        self.runtime_funcs.insert("lift_struct_eq".to_string(), func_id);

        // lift_output_struct(*const LiftStruct)
        let mut sig = self.module.make_signature();
        sig.params.push(AbiParam::new(pointer_type));
        let func_id = self.module
            .declare_function("lift_output_struct", cranelift_module::Linkage::Import, &sig)
            .map_err(|e| format!("Failed to declare lift_output_struct: {}", e))?;
        self.runtime_funcs.insert("lift_output_struct".to_string(), func_id);
        ...
        self.runtime_funcs.insert("lift_struct_free".to_string(), func_id);
    ...
    fn compile_expr_static(
        ...
        match expr {
            ...
            Expr::StructLiteral { type_name, fields } => {
                Self::compile_struct_literal(builder, type_name, fields, symbols, runtime_funcs, user_func_refs, variables)
            }

            Expr::FieldAccess { expr, field_name } => {
                Self::compile_field_access(builder, expr, field_name, symbols, runtime_funcs, user_func_refs, variables)
            }

            Expr::FieldAssign { expr, field_name, value, .. } => {
                Self::compile_field_assign(builder, expr, field_name, value, symbols, runtime_funcs, user_func_refs, variables)
            }
    ...
    fn compile_binary_expr(
        ...
        let is_struct_op = matches!(left_type, Some(DataType::Struct(_))) || matches!(right_type, Some(DataType::Struct(_)));
        if is_struct_op && matches!(op, Operator::Eq | Operator::Neq) {
            ...
            match op {
                Operator::Eq => {
                    // Struct equality
                    let func_ref = runtime_funcs.get("lift_struct_eq")
    ...
    fn compile_struct_literal(
        ...
        let type_name_slot = builder.create_sized_stack_slot(cranelift_codegen::ir::StackSlotData::new(
            cranelift_codegen::ir::StackSlotKind::ExplicitSlot,
        ...
        for (i, byte) in type_name_bytes.iter().enumerate() {
            let byte_val = builder.ins().iconst(types::I8, *byte as i64);
            builder.ins().stack_store(byte_val, type_name_slot, i as i32);
        }
        // Null terminator
        let null_byte = builder.ins().iconst(types::I8, 0);
        builder.ins().stack_store(null_byte, type_name_slot, type_name_bytes.len() as i32);
        ...
        let func_ref = runtime_funcs.get("lift_struct_new")
        ...
        let set_field_ref = runtime_funcs.get("lift_struct_set_field")
        ...
        for (field_name, field_expr) in fields {
            ...
            let field_name_slot = builder.create_sized_stack_slot(cranelift_codegen::ir::StackSlotData::new(
                cranelift_codegen::ir::StackSlotKind::ExplicitSlot,
            ...
            for (i, byte) in field_name_bytes.iter().enumerate() {
                let byte_val = builder.ins().iconst(types::I8, *byte as i64);
                builder.ins().stack_store(byte_val, field_name_slot, i as i32);
            }
            let null_byte = builder.ins().iconst(types::I8, 0);
            builder.ins().stack_store(null_byte, field_name_slot, field_name_bytes.len() as i32);

            let field_name_ptr = builder.ins().stack_addr(types::I64, field_name_slot, 0);

            // Compile field value
            let field_val_raw = Self::compile_expr_static(builder, field_expr, symbols, runtime_funcs, user_func_refs, variables)?
                .ok_or_else(|| format!("Struct field '{}' must produce a value", field_name))?;

            // Determine field type for type tag
            let field_type = crate::semantic::determine_type_with_symbols(field_expr, symbols, 0)
            ...
            let field_val = match field_type_resolved {
                DataType::Flt => {
                    // Bitcast f64 to i64 for storage
                    builder.ins().bitcast(types::I64, MemFlags::new(), field_val_raw)
                }
                DataType::Bool | DataType::Int | DataType::Str | DataType::List { .. } |
                DataType::Map { .. } | DataType::Range(_) | DataType::Struct(_) => {
            ...
            let type_tag = builder.ins().iconst(types::I8, data_type_to_type_tag(&field_type_resolved) as i64);

            // Call lift_struct_set_field(struct, field_name, type_tag, value)
            builder.ins().call(*set_field_ref, &[struct_ptr, field_name_ptr, type_tag, field_val]);
        }

        Ok(Some(struct_ptr))
    }

    /// Compile field access (struct.field)
    fn compile_field_access(
        ...
        let struct_val = Self::compile_expr_static(builder, expr, symbols, runtime_funcs, user_func_refs, variables)?
        ...
        let func_ref = runtime_funcs.get("lift_struct_get_field")
    ...
    /// Compile field assignment (struct.field := value)
    fn compile_field_assign(
        ...
        // Compile new value
        let new_val_raw = Self::compile_expr_static(builder, value, symbols, runtime_funcs, user_func_refs, variables)?
        ...
        let new_val = match value_type_resolved {
            DataType::Flt => {
                builder.ins().bitcast(types::I64, MemFlags::new(), new_val_raw)
            }
            DataType::Bool | DataType::Int | DataType::Str | DataType::List { .. } |
            DataType::Map { .. } | DataType::Range(_) | DataType::Struct(_) => {
                new_val_raw
        ...
        let type_tag = builder.ins().iconst(types::I8, data_type_to_type_tag(&value_type_resolved) as i64);

        // Call lift_struct_set_field(struct, field_name, type_tag, new_value)
        let func_ref = runtime_funcs.get("lift_struct_set_field")
            .ok_or_else(|| "Runtime function lift_struct_set_field not found".to_string())?;
        builder.ins().call(*func_ref, &[struct_ptr, field_name_ptr, type_tag, new_val]);
