...
fn run_lift_file(file_path: &str) -> Result<Expr, String> {
    let code = fs::read_to_string(file_path).map_err(|e| format!("Failed to read file: {e}"))?;
...
fn test_typecheck_functions() {
    ...
    let mut ast = parser
        .parse("function add(x: Int, y: Int): Int { x + y }")
        .unwrap();
    assert!(ast.prepare(&mut symbols).is_ok());
...
fn test_interpreter_string_operations() {
    ...
    assert_eq!(
        result,
        Expr::Literal(LiteralData::Str("'Hello World'".into()))
    );
}
...
fn test_interpreter_if_else() {
    ...
    let mut ast = parser
        .parse("{let x = 5; if x > 3 { x * 2 } else { x }}")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    assert_eq!(result, Expr::Literal(LiteralData::Int(10)));
...
fn test_interpreter_functions() {
    ...
    let mut ast = parser
        .parse("{function square(x: Int): Int { x * x }; square(x: 5)}")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    ...
    let mut ast = parser
        .parse("{function add(x: Int, y: Int): Int { x + y }; add(x: 3, y: 7)}")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    assert_eq!(result, Expr::Literal(LiteralData::Int(10)));
...
fn test_negative_numbers() {
    ...
    let mut ast = parser
        .parse("{function abs(x: Int): Int { if x < 0 { 0 - x } else { x } }; abs(x: -10)}")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    assert_eq!(result, Expr::Literal(LiteralData::Int(10)));
...
fn test_interpreter_nested_blocks() {
    ...
    let mut ast = parser
        .parse("{let x = 5; let result: Int = {let x = 10; x}; result + x}")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    assert_eq!(result, Expr::Literal(LiteralData::Int(15)));
...
fn test_interpreter_type_inference() {
    ...
    let mut ast = parser
        .parse("{let x = 5; let y = 2.5; let z = true; z}")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    assert_eq!(result, Expr::Literal(LiteralData::Bool(true)));
...
fn test_interpreter_nested_if_else() {
    ...
    assert_eq!(
        result,
        Expr::Literal(LiteralData::Str("'both true'".into()))
    );
}
...
fn test_map_literals() {
    ...
    match result {
        Expr::RuntimeMap { data, .. } => {
            assert_eq!(data.len(), 3);
            assert_eq!(
                data.get(&KeyData::Int(1)),
                Some(&Expr::Literal(LiteralData::Str("'one'".into())))
            );
            assert_eq!(
                data.get(&KeyData::Int(2)),
                Some(&Expr::Literal(LiteralData::Str("'two'".into())))
            );
            assert_eq!(
                data.get(&KeyData::Int(3)),
                Some(&Expr::Literal(LiteralData::Str("'three'".into())))
            );
        }
    ...
    match result {
        Expr::RuntimeMap { data, .. } => {
            assert_eq!(data.len(), 2);
            assert_eq!(
                data.get(&KeyData::Str("'hello'".into())),
                Some(&Expr::Literal(LiteralData::Int(1)))
            );
            assert_eq!(
                data.get(&KeyData::Str("'world'".into())),
                Some(&Expr::Literal(LiteralData::Int(2)))
            );
        }
        _ => panic!("Expected RuntimeMap, got {:?}", result),
    }
    ...
    match result {
        Expr::RuntimeMap { data, .. } => {
            assert_eq!(data.len(), 2);
            assert_eq!(
                data.get(&KeyData::Bool(true)),
                Some(&Expr::Literal(LiteralData::Str("'yes'".into())))
            );
            assert_eq!(
                data.get(&KeyData::Bool(false)),
                Some(&Expr::Literal(LiteralData::Str("'no'".into())))
            );
        }
        _ => panic!("Expected RuntimeMap, got {:?}", result),
    }
    ...
    match result {
        Expr::RuntimeMap { data, .. } => {
            ...
            assert_eq!(
                data.get(&KeyData::Int(1)),
                Some(&Expr::Literal(LiteralData::Int(15)))
            );
            assert_eq!(
                data.get(&KeyData::Int(2)),
                Some(&Expr::Literal(LiteralData::Int(40)))
            );
            assert_eq!(
                data.get(&KeyData::Int(3)),
                Some(&Expr::Literal(LiteralData::Int(25)))
            );
        }
        _ => panic!("Expected RuntimeMap, got {:?}", result),
    }
...
fn test_collections_in_variables() {
    ...
    let mut ast = parser
        .parse("{let mymap = #{1: 'one', 2: 'two'}; mymap}")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    match result {
    ...
    let mut ast = parser
        .parse("{let x = 10; let y = 20; [x, y, x + y]}")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    match result {
...
fn test_list_indexing() {
    ...
    let mut ast = program
        .parse("let nums = [100, 200, 300]; let i = 1; nums[i]")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
...
fn test_list_indexing_type_checking() {
    ...
    let program = grammar::ProgramParser::new();
    let mut ast = program
        .parse("let nums = [1, 2, 3]; let x = nums[0]")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    ...
    assert!(result.unwrap_err()[0]
        .to_string()
        .contains("List index must be of type Int"));
    ...
    assert!(result.unwrap_err()[0]
        .to_string()
        .contains("Cannot index into type"));
}
...
fn test_list_indexing_runtime_errors() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Index 5 out of bounds for list of length 3"));
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Index -1 out of bounds"));
}
...
fn test_map_indexing() {
    ...
    let program = grammar::ProgramParser::new();
    let mut ast = program
        .parse("let m = #{'x': 100, 'y': 200}; let k = 'x'; m[k]")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    assert_eq!(result, Expr::Literal(LiteralData::Int(100)));
    ...
    let mut ast = parser
        .parse("#{'outer': #{'inner': 42}}['outer']['inner']")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
    let result = ast.interpret(&mut symbols, 0).unwrap();
    assert_eq!(result, Expr::Literal(LiteralData::Int(42)));
...
fn test_map_indexing_type_checking() {
    ...
    assert!(result.unwrap_err()[0]
        .to_string()
        .contains("Map key must be of type Int"));
    ...
    assert!(result.unwrap_err()[0]
        .to_string()
        .contains("Map key must be of type Str"));
}
...
fn test_map_indexing_runtime_errors() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Key Int(3) not found in map"));
...
fn test_not_operator() {
    ...
    assert!(result.unwrap_err()[0]
        .to_string()
        .contains("Not operator requires boolean operand"));
}
...
fn test_if_expression_type_inference() {
    ...
    let mut ast = parser
        .parse("let y = if false { 'hello' } else { 'world' }")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();

    // Test 3: Type mismatch should fail
    let mut ast = parser
        .parse("let z = if true { 42 } else { 'string' }")
        .unwrap();
    let result = ast.prepare(&mut symbols);
    assert!(result.is_err());
    assert!(result.unwrap_err()[0]
        .to_string()
        .contains("Cannot infer type"));
    ...
    let mut symbols3 = SymbolTable::new();
    let mut ast = parser
        .parse("function test(): Int { if true { 1 } else { 2 } }")
        .unwrap();
    ast.prepare(&mut symbols3).unwrap();

    // Test 6: Nested if expressions
    let mut ast = parser
        .parse("let grade = if 85 >= 90 { 'A' } else if 85 >= 80 { 'B' } else { 'C' }")
        .unwrap();
    ast.prepare(&mut symbols).unwrap();
}
